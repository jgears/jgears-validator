...
public class AnyValidator<T> extends Validator<T> {
	
	private String message;
	private List<Validator<T>> validators = new ArrayList<>();
	
	public AnyValidator(String message) {
		this.message = message;
	}

	public AnyValidator<T> add(Validator<T> validator) {
		if (validator != null) {
	...
	public void validate(T value, List<String> errors) {
		...
		List<String> savedErrors = new ArrayList<>();
		...
		for (Validator<T> v : validators) {
			...
			if (tempErrors.isEmpty()) {
				...
				if (savedErrors.isEmpty()) {
					savedErrors.addAll(tempErrors);
				}
				tempErrors.clear();
			}
		}
		...
		if (!savedErrors.isEmpty()) {
			...
			else { errors.addAll(savedErrors); }
		}
	}
